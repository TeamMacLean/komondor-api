...
const schema = new Schema(
  {
    ...
    // Must stay a real schema path: Mongoose silently drops an unknown flag, and
    // the post-save hook would then move the file a second time. Same in Read.js.
 ...
 * @returns {Promise<*>} Resolves once the file is in place, or immediately if it was already moved.
 */
const movePostSave = async function (doc) {
  // lib/file-utils.js moves the file before writing the row; moving again here
  // would look for a source that is no longer in staging.
  if (doc.skipPostSave) {
    return;
  }

  // On any later save doc.file.path is already datastore-relative, so a second
  // move would look for a source that is not there.
  if (!doc.wasNew) {
    return;
  }
  ...
  return Promise.all([prom, doc.populate("file").execPopulate()])
    .then((out) => {
      ...
      return parent.getRelativePath().then((relPath) => {
        relPath = _path.join(relPath, "additional");
        const relPathWithFilename = _path.join(
          relPath,
          additionalFile.file.originalName,
        );
...
// Exposed as a static so the guards above are testable without a DB connection.
