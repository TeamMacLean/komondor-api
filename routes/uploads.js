...
const trackActiveUpload = (req, res, next) => {
  ...
  // Cleanup and resume use one synchronous in-process exclusion protocol. If
  // the abandoned-upload sweep claimed this id first, do not enter tus while
  // its blob may be unlinked; 410 tells a resumable client to start afresh.
  if (!quota.beginRequest(uploadId)) {
    return res.status(410).send({ error: "Upload is no longer available" });
  }
