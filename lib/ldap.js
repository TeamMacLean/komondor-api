...
// Every attribute the app reads off the LDAP user record. Requested
// explicitly so group resolution never depends on the server's default
// attribute set. Attribute names are matched case-insensitively by the
// server, so "memberOf" also covers directories that store "memberof".
//
// getUserForToken reads displayName, givenName, sn, mail and company;
// Group.GroupsIAmIn reads memberOf and falls back through sAMAccountName,
// uid and mailNickname to identify the username.
...
exports.USER_SEARCH_ATTRIBUTES = USER_SEARCH_ATTRIBUTES;
...
exports.authenticate = function authenticate(username, password) {
  return new Promise((good, bad) => {
    ...
    const options = {
      ...
      searchAttributes: USER_SEARCH_ATTRIBUTES,
    };
      ...
      (bindErr) => {
        ...
        client.search(LDAP_SEARCH_BASE, searchOptions, (searchErr, res) => {
          ...
          res.on("searchEntry", (entry) => {
            // ldapjs 2.x exposes the entry as `.object` (`.pojo` is the 3.x
            // API and is undefined here, which left every field blank).
