...
} = require("./ingest-payload-validation");
const { assertProjectAcceptsHpcWrites } = require("./storage-state");
...
const failJob = async (jobId, error, options = {}) => {
  ...
  if (retry && job.attempts < job.maxAttempts) {
    ...
    console.warn(
      `[Ingest Queue] Job ${jobId} for run ${job.runId} failed on attempt ${
        job.attempts
      }/${job.maxAttempts}, retrying in ${Math.round(
        backoffMs / 1000,
      )}s: ${message}`,
    );
    return;
  ...
  try {
    await Run.findByIdAndUpdate(job.runId, {
      $set: {
        status: "error",
        statusError:
          error && error.runStatusError ? error.runStatusError : message,
      },
    });
  } catch (updateError) {
    console.error(
...
const planRawFileStage = async (runId, rawFiles) => {
  ...
  if (toMove.length !== rawFiles.length) {
    ...
    console.log(
      `[Ingest Queue] Run ${runId}: ${already} of ${
        rawFiles.length
      } raw file(s) already in the datastore; ${
        toMove.length > 0
 ...
 *
 * `sibling` is the contract for BOTH upload methods. `rowID` was a test-only
 * invention no real client emitted; supporting both produced method-specific
 * fallback gaps where validation accepted a pair that this function dropped.
 * Invalid stored legacy shapes fail loudly here rather than completing a run
 * with missing relationships.
 *
 * @returns {Array<Array<string>>} [readName, siblingName] pairs.
 */
const siblingLinks = (rawFiles) => {
  ...
  files.forEach((file) => {
    if (names.has(file.name)) {
      throw new Error(
        `Duplicate raw file name "${file.name}" in pairing payload`,
      );
    }
    names.add(file.name);
    if (file.rowID !== undefined) {
      throw new Error(
        `Raw file "${file.name}" uses unsupported rowID pairing; send reciprocal sibling names`,
      );
    }
  });
  ...
  files.forEach((file) => {
    ...
    if (file.indexed === true && hasSibling) {
      throw new Error(
        `Raw file "${file.name}" is an indexed read and cannot declare a sibling`,
      );
    }
    if (file.paired === true && !hasSibling) {
      throw new Error(
        `Raw file "${file.name}" is marked paired but names no sibling`,
      );
    }
    if (file.paired === false && hasSibling) {
      throw new Error(
        `Raw file "${file.name}" names a sibling but is marked unpaired`,
      );
    }
    if (!hasSibling) {
      return;
    ...
    if (file.sibling === file.name) {
      throw new Error(
        `Raw file "${file.name}" names itself as its own sibling`,
      );
    }

    const mate = byName.get(file.sibling);
    if (!mate || mate.sibling !== file.name) {
      throw new Error(
        `Raw file "${file.name}" does not have a reciprocal sibling declaration`,
      );
    }
    links.push([file.name, file.sibling]);
  });
 ...
 *   happen to exist yet.
 * @param {object} _uploadInfo - Retained in the stage signature for callers;
 *   pairing itself is method-independent.
...
const finaliseReadStage = async (
  ...
  // The names this payload speaks for. Anything else on the run is untouched.
  ...
  const updates = [];
  // The links the payload describes now, as read name -> sibling Read id.
  // Built before any write so the reconciliation below can compare the whole
  // desired state against what is stored, rather than only adding to it.
  const wanted = new Map();

  siblingLinks(rawFiles).forEach(([name, siblingName]) => {
    const read = readsByName.get(name);
    const sibling = readsByName.get(siblingName);

    if (!read || !sibling) {
      // Every raw-file move has settled before finalisation. A declared mate
      // missing here is therefore an invariant failure, not an optional link:
      // completing would permanently turn a paired payload into unpaired data.
      throw new Error(
        `Run ${runId}: cannot link ${name} to sibling ${siblingName} — no ingested read for ${
          read ? siblingName : name
        }`,
      );
    }

    wanted.set(name, sibling._id);
  });

  // A resolved move owes one Read for every raw-file entry, paired or not.
  // The declared-pair check above keeps its more useful mate-specific error;
  // this catches the case it could not see before, where an impossible
  // empty/partial result for an unpaired move still marked the run complete.
  rawFileNames.forEach((name) => {
    if (!readsByName.has(name)) {
      throw new Error(
        `Run ${runId}: cannot finalise raw file "${name}" — no ingested Read`,
      );
    }
  });

  // Reconcile, do not merely add. This used to only ever SET a sibling, so a
  // link the payload no longer describes stayed on the Read forever: an audit
  // reingested a run with the pairing removed, got 200 and a "complete" run,
  // and found the delivered Read still carrying paired:true with a sibling
  // pointing at a file that is no longer its mate. A reingest is a statement
  // of what the run should be, so a removed link has to be removed.
  //
  // Scoped to reads this payload actually names: a run's other reads are not
  // this payload's business.
  readsByName.forEach((read, name) => {
    if (!rawFileNames.has(name)) {
      return;
    }

    const desired = wanted.get(name);
    const current = String(read.sibling || "");
    const currentPaired = read.paired;

    if (desired === undefined) {
      if (current !== "" || currentPaired !== false) {
        updates.push(
          Read.updateOne(
            { _id: read._id },
    ...
    if (current !== String(desired) || currentPaired !== true) {
      updates.push(
        Read.updateOne(
          { _id: read._id },
          { $set: { sibling: desired, paired: true } },
        ),
      );
    }
  });
...
const pendingAdditionalFiles = async (runId, additionalFiles) => {
  ...
  if (remaining.length !== additionalFiles.length) {
    console.log(
      `[Ingest Queue] Run ${runId}: skipping ${
        additionalFiles.length - remaining.length
      } already-ingested additional file(s)`,
    );
  }
...
const runIngestJob = async (job) => {
  ...
  const sample = await Sample.findById(run.sample).select("project");
  if (!sample || !sample.project) {
    throw new Error(
      `Cannot ingest run ${run._id}: its parent sample/project no longer exists`,
    );
  }
  const project = await Project.findById(sample.project).select("storage");
  if (!project) {
    throw new Error(
      `Cannot ingest run ${run._id}: project ${sample.project} no longer exists`,
    );
  }
  assertProjectAcceptsHpcWrites(project);

  const payload = job.payload || {};
  const { rawFiles, additionalFiles, rawFilesUploadInfo } = payload;

  const rawFileErrors = validateStoredRawFiles(rawFiles);
  if (rawFileErrors.length > 0) {
    throw new Error(
      `Cannot ingest run ${
        run._id
      }: stored raw-file payload is invalid: ${rawFileErrors.join("; ")}`,
    );
  }

  // The HTTP boundary rejects LibraryType contradictions, but durable jobs
  // outlive deploys and can also predate that boundary or be repaired
  // directly. Repeat the cheap semantic invariant here before moving bytes so
  // a stale/seeded paired-indexed job cannot finish with no index read (or a
  // paired Run with unpaired Reads).
  if (!run.libraryType) {
    throw new Error(
      `Cannot ingest run ${run._id}: the Run has no library type`,
    );
  }
  ...
  if (!selectedLibraryType) {
    throw new Error(
      `Cannot ingest run ${run._id}: unknown library type "${run.libraryType}"`,
    );
  }
  ...
  if (libraryTypeErrors.length > 0) {
    throw new Error(
      `Cannot ingest run ${run._id}: payload contradicts library type "${
        run.libraryType
  ...
  const processOne = async () => {
    ...
    if (job.attempts > job.maxAttempts) {
      ...
      await failJob(
        job._id,
        new Error(
          `Ingest gave up after ${
            job.attempts - 1
          } attempt(s); the last one did not report back`,
        ),
