 ...
 * @returns {Promise<Set<string>>} Delivered original names.
 */
const deliveredFileNames = async (runId) => {
  ...
  const delivered = await ingestedByName([...reads, ...additional]);
  return new Set(delivered.keys());
};
 ...
 * already ingested by an earlier attempt: an hpc-mv file names its sibling, a
 * local-filesystem upload pairs on rowID, and each link is directed.
 * finaliseReadStage is the only caller and the only place a raw file gets
 * paired — file-utils.js used to keep a second copy of these two rules for
 * its own within-batch pairing, which is what let a partial retry's pairing
 * silently go wrong; that copy is gone.
 *
