...
const checkUploadAllowed = async ({ id, username, size, directory, now }) => {
  ...
  if (free !== null) {
    ...
    // An upload of unknown length is charged nothing here for itself: charging
    // the maximum would refuse every upload on a volume smaller than it.
    const ownCharge = Number.isFinite(size) ? size : 0;
    const projected = free - reservedByOthers - ownCharge;
...
const assertUploadComplete = async (directory, id) => {
  ...
  const { size, offset } = info;

  // Missing/NaN size covers deferred-length uploads that never got a final
  // size: there is no target to compare offset against, so it cannot be done.
  if (!Number.isFinite(size) || !Number.isFinite(offset) || offset !== size) {
    throw new Error(
      `Upload ${id} is not complete: offset ${offset} of declared size ${size}`,
    );
  }

  let stats;
  try {
    stats = await fs.stat(_path.join(directory, id));
  } catch (err) {
    throw new Error(`Upload ${id} has no blob on disk: ${err.message}`);
  }

  // Trusts the bytes actually written, not just the sidecar: a disk write
  // that stalled or was truncated can leave a sidecar claiming completion the
  // blob itself never reached.
  if (stats.size !== size) {
    throw new Error(
      `Upload ${id} blob is ${stats.size} bytes on disk, sidecar declares ${size}`,
    );
  }
};
...
const recoverUploadReservations = async (directory) => {
  ...
  for (const id of ids) {
    ...
    registerUpload({ id, username: owner, size: info.size });
    recovered += 1;
  }
