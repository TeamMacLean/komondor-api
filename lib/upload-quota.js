...
// so both idle pruning and abandoned-file cleanup can tell "nobody has touched
// this in an hour" from "this is streaming right now", which a timestamp
// stamped at request start cannot express.
const activeRequests = new Map();

// Upload ids a cleanup sweep has committed to remove. This is separate from
// activeRequests because the two states have opposite meanings: an active
// request makes cleanup stand down, while a cleanup claim makes a later
// request stand down. Both transitions are synchronous, so one side always
// wins before either begins asynchronous filesystem work.
 ...
 * @returns {boolean} False when cleanup has already claimed this upload.
 */
const beginRequest = (id) => {
  if (!id) {
    return false;
  }
  if (cleanupClaims.has(id)) {
    return false;
  }
...
/**
 * Atomically, relative to beginRequest's synchronous transition, commits a
 * sweep to deleting one upload. Once claimed, a later request is refused by
 * beginRequest rather than entering tus while unlink is pending.
 * @param {string} id - The tus upload id.
 * @returns {boolean} Whether cleanup acquired exclusive ownership.
 */
const claimForCleanup = (id) => {
  if (!id || activeRequests.has(id) || cleanupClaims.has(id)) {
    return false;
  }
  cleanupClaims.add(id);
  return true;
};

/** Releases a cleanup claim after deletion succeeds or fails. */
const releaseCleanupClaim = (id) => cleanupClaims.delete(id);
...
const checkUploadAllowed = async ({ id, username, size, directory, now }) => {
  ...
  if (usage.bytes + charge > limits.maxInflightBytesPerUser) {
    return {
      allowed: false,
      status: 429,
      error: `User '${username}' would have ${
        usage.bytes + charge
      } bytes of uploads in progress (limit ${limits.maxInflightBytesPerUser})`,
    };
  }
...
/** Test helper: forgets all in-process upload state. */
const clearUploads = () => {
  uploads.clear();
...
const cleanupAbandonedUploads = async ({
  ...
  for (const id of candidates) {
    // A request that is still open is not abandoned, however old its blob or
    // registration looks. Large/paused PATCHes can legitimately cross the
    // 48-hour sweep threshold without another write or timestamp touch.
    if (activeRequests.has(id)) {
      continue;
    }
    ...
    try {
      ...
      // stat/readUploadState both yield to the request loop. Claim immediately
      // before deletion, with no await between the active check and the claim.
      // A request that won first makes this return false; a request arriving
      // afterward sees the claim in beginRequest and is refused before tus.
      if (!claimForCleanup(id)) {
        continue;
      }
