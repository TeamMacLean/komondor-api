...
// Read per-call, not destructured once: a test that sets NODE_ENV after this
// module is first required would otherwise still reach the real SMTP host.
const isNonSendingEnv = () =>
...
const actualSending = async (mailObj) => {
  ...
  // Never send real mail from development or from a test run. The integration
  // suite drives the genuine ingest path, which emails on success; without
  // this it attempted the real SMTP host and only "passed" because the DNS
  // failure was swallowed downstream.
  if (isNonSendingEnv()) {
    console.log("\n========== EMAIL (NOT SENT) ==========");
...
try {
  ...
  console.warn(
    "Could not parse ENA_ADMIN_EMAILS as JSON array, using raw string",
  );
}
...
const sendMd5VerificationEmail = async (data) => {
  ...
  message += `Please investigate and if you think you have made an error with the provided MD5 checksum, please independently verify a new figure and then email the webmaster deeks@nbi.ac.uk with this value to update.\n\n`;
