 ...
 *
 * They previously did not. routes/uploads.js and lib/fileUpload.js honoured an
 * UPLOAD_DIRECTORY override while lib/file-utils.js and models/File.js
 * hardcoded <cwd>/files, so setting that variable in production would have
 * stranded every finished upload somewhere nothing looked for it — and the
 * failure would have surfaced as a confusing "refusing upload" from the path
 * guard rather than as a misconfiguration.
 *
