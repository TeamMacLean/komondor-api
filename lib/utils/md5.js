...
const calculateFileMd5 = async (target, onTick) => {
  ...
  //
  // start: 0 because "the MD5 of a file" must not depend on where the
  // caller's descriptor happens to be positioned. A handle that has already
  // been streamed once sits at EOF, and without this the hash came back as
  // the digest of zero bytes — measured, and silently wrong rather than an
  // error, which is the worst way for a checksum to fail.
