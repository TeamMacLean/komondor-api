...
// second-guess an operator. The queue's own defaults — a 5s poll and a 5min
// lease (DEFAULT_INTERVAL_MS / DEFAULT_LEASE_MS in lib/ingest-queue.js) — sit
// well inside them. The comment here used to say "5s, 60min"; the lease default
// is 5 minutes, deliberately short, because it is a heartbeat lease rather than
// an estimate of how long a job takes.
//
// Known gap: these are two independent range checks, so the pair
// INGEST_POLL_MS=300000 (the max) with INGEST_LEASE_MINUTES=1 (the min) passes
// while configuring a lease that expires before the poll meant to renew it —
// every job would be reclaimed from a worker that is still running it.
// startIngestWorker clamps that at runtime via MIN_LEASE_POLLS, so it is not a
// live fault; a cross-check here would catch it at boot instead of silently
// overriding what the operator asked for.
