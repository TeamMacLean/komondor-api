...
const batchFailure = (settled, total) => {
  ...
  const error = new Error(
    `${
      reasons.length
    } of ${total} file(s) in this batch failed to process: ${messages.join(
      "; "
    )}`
  );
...
/**
 * Hashes a retained HPC source, following a permitted leaf symlink.
 *
 * calculateFileMd5 opens O_NOFOLLOW, so a symlinked leaf raises ELOOP — and a
 * bare `.catch(() => null)` around it turns "this is a link" into "the content
 * could not be verified", which the caller reads as "do not adopt". Reproduced
 * by execution: an hpc-mv claim on a permitted leaf symlink with no declared
 * MD5, whose Read save then failed, could never be reconciled — every retry
 * stalled on "destination already exists" with zero Reads written, for a file
 * shape the move itself explicitly supports (models/File.js openPinnedSource
 * takes the same ELOOP fallback).
 *
 * The fallback re-vouches for the target rather than trusting the link: only a
 * realpath that still sits inside the HPC inbox (or an operator-configured
 * ALLOWED_LINK_ROOTS entry, which assertWithinReal honours) is hashed, and it
 * is opened O_NOFOLLOW again so a second layer of link introduced since is
 * still refused.
 *
 * @param {string} sourcePath - The retained source path.
 * @returns {Promise<string|null>} The MD5, or null when it cannot be taken.
 */
const md5OfRetainedSource = async (sourcePath) => {
  try {
    return await calculateFileMd5(sourcePath);
  } catch (err) {
    if (err.code !== "ELOOP") {
      return null;
    }

    const permitted = await assertWithinReal(
      process.env.HPC_TRANSFER_DIRECTORY,
      sourcePath
    );
    if (!permitted) {
    ...
    const realTarget = await fs.realpath(sourcePath).catch(() => null);
    if (!realTarget) {
      return null;
    }
    return await calculateFileMd5(realTarget).catch(() => null);
  }
};
...
const adoptAlreadyMovedFile = async (
  ...
  // was moved yet", so instead this checks that the retained source is still
  // present and the destination is a complete copy of it, by size. moveTo-
  // FolderAndSave copies an hpc source (fs.copyFile, an independent inode) —
  // NOT a hard link — so an inode comparison would never match a genuine
  // success; size is inode-agnostic and holds for both the same-device and
  // cross-device cases. The destination path is this run's own
  // .../<sample>/<run>/raw/<name>, so a same-size unrelated file cannot land
  // there by accident, and condition 4 re-hashes the destination against the
  // declared MD5 when the request carried one.
  if (sourceMayPersist) {
    const sourceStats = await fs.stat(sourcePath).catch(() => null);
    if (!sourceStats || sourceStats.size !== destinationStats.size) {
      return null;
    }
  } else {
  ...
  // Condition 4. Content, not just size, must match before adopting.
  //
  // When the request declared an MD5, that is the stronger proof and is
  // checked against the destination directly. When it did not, size alone
  // is not enough: reproduced by execution — a first attempt copies "AAAA",
  // its save fails, the retained source is then corrected in place to a
  // same-size "BBBB" (a legitimate re-submission of a fixed file under the
  // same name), and a size-only retry adopted the OLD "AAAA" destination as
  // though it were the correct bytes. That is silent data corruption, worse
  // than the stall this reconciliation exists to remove. So absent a
  // declared MD5, the retained source and the destination are hashed and
  // compared directly — content, not merely length, must agree.
  if (expectedMd5) {
    const actualMd5 = await calculateFileMd5(destination).catch(() => null);
    if (!actualMd5 || actualMd5.toLowerCase() !== expectedMd5.toLowerCase()) {
      return null;
    }
  } else if (sourceMayPersist) {
    const [sourceMd5, destinationMd5] = await Promise.all([
      md5OfRetainedSource(sourcePath),
      calculateFileMd5(destination).catch(() => null),
    ]);
...
const moveIntoDatastore = async (
  ...
  try {
    ...
    if (!adopted) {
      ...
      // The File row was saved BEFORE the move was attempted, so a move that
      // never happened leaves a document referenced by nothing — no Read, no
      // AdditionalFile — pointing at a path with no bytes at it. The unique
      // { name, path, createFileDocumentId } index then makes that orphan
      // permanent: an audit reproduced an hpc-mv claim for a file that was
      // not there yet, and once the scientist did put it there, every retry
      // died on E11000 against the orphan rather than moving anything.
      //
      // Deleted only in the branch where nothing was adopted and nothing
      // moved, and only for this attempt's own freshly-created document —
      // adoptAlreadyMovedFile returning a row means the bytes ARE in the
      // datastore and the document must survive.
      try {
        await File.deleteOne({ _id: savedFile._id });
      } catch (cleanupError) {
        console.error(
          `[File Processing] Could not remove the unreferenced File row for '${relPathWithFilename}' after a failed move:`,
          cleanupError
        );
      }
...
async function processSingleReadFile(
  ...
  const readData = {
    ...
    file: fileForRow._id,
    // Reciprocal `sibling` is the one pairing contract for every source.
    // The route has already checked that an explicit paired flag agrees.
    paired: typeof file.sibling === "string",
