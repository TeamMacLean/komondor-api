...
const verifyReadMd5 = async (read, run) => {
  ...
  try {
    ...
    //
    // `originalName` is client-supplied: it arrives as tus Upload-Metadata.
    // lib/file-utils.js reduces it with safeBasename before a File document is
    // written, and lib/file-utils.js builds the datastore destination the same
    // way this does — but documents predating that guard were never sanitised
    // and this job reads them back. A bare path.join would let a stored
    // "../../../etc/shadow" have this hash whatever the process can read, and
    // write the digest into the Read document: an oracle for the contents of
    // files outside the datastore.
    //
    // Both halves of the guard are needed. safeBasename is the invariant every
    // File path component is meant to satisfy, and it is what stops a name
    // that traverses *within* the datastore — "../../other-group/reads.fq"
    // stays under the root, so containment alone would allow it and leak
    // another group's checksum. resolveWithinReal then vouches for the whole
    // join, symlinks included, exactly as models/File.js does for the move
    ...
    if (!destinationPath) {
      // Thrown, not skipped: caught below and reported as an error for this
      // read, which marks the run's verification failed. A read whose stored
      // name cannot be trusted must not be quietly recorded as verified.
      throw new Error(
