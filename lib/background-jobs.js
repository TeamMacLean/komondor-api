...
/**
 * Sweeps upload fragments that were started and never finished.
 *
 * Deliberately conservative: `includeCompleted` and `includeOrphans` are left
 * false, so this only ever deletes *incomplete* uploads past
 * UPLOAD_ABANDONED_HOURS. A finished upload sitting in the directory is not
 * rubbish — it is a file the user uploaded and has not yet attached to a
 * project — and an orphaned blob with no sidecar is every upload the old
 * unauthenticated server accepted. Both are counted and logged for an operator
 * to look at rather than removed on a timer.
 *
 * Runs daily.
 */
...
const initializeBackgroundJobs = () => {
  ...
  // Abandoned upload sweep - daily at 3:00 AM, an hour after the run cleanup
  // so the two are not competing for the same disk at the same moment.
  scheduledTasks.push(
