...
const { cleanupAbandonedUploads } = require("./upload-quota");
...
// How long a run may sit in "in_progress" before it is treated as stranded.
const STALLED_VERIFICATION_MINUTES = 60;

// Idle runs stay quiet, but log hourly: total silence makes "idle" and "no
// longer running" look identical.
...
const processMd5Verification = async () => {
  ...
  try {
    ...
    try {
      const recovery = await recoverStalledVerifications(
        STALLED_VERIFICATION_MINUTES,
      );
    ...
    if (runs.length === 0) {
      ...
      // A negative gap means the wall clock stepped backwards; treat as due.
    ...
    for (const run of runs) {
      ...
      if (result.success === false && !result.shouldRetry) {
        console.error(
          `[Background Job] Hard failure verifying run ${run._id}. Marked as failed.`,
        );
      }

      // Email only when verification finished with mismatches or read errors.
      if (
        !result.skipped &&
        (result.mismatches > 0 || (result.errors && result.errors > 0))
      ) {
        try {
...
/**
 * Sweeps upload fragments that were started and never finished. Runs daily.
 *
 * `includeCompleted` and `includeOrphans` stay false: a finished upload is a
 * file the user has not attached to a project yet, not rubbish. Those are
 * counted and logged for an operator rather than removed on a timer.
 */
const processUploadSweep = async () => {
  if (uploadSweepRunning) {
    console.log("[Background Job] Upload sweep already running, skipping");
    return;
  }
  ...
  try {
    ...
    console.log(
      `[Background Job] Starting abandoned upload sweep in ${directory}`,
    );

    const result = await cleanupAbandonedUploads({ directory });

    console.log(
      `[Background Job] Upload sweep completed: ${result.removed.length} removed, ` +
        `${(result.sidecars || []).length} orphaned sidecars removed, ` +
        `${result.completed.length} finished-but-unattached left in place, ` +
        `${result.orphans.length} orphaned blobs left in place`,
    );

    if (result.errors.length > 0) {
      console.error(
        "[Background Job] Upload sweep could not process:",
        result.errors,
      );
    }
  } catch (error) {
    console.error("[Background Job] Error during upload sweep:", error);
  } finally {
...
const initializeBackgroundJobs = () => {
  ...
  // MD5 verification job - runs every 5 minutes. The tick itself is not logged;
  // that would defeat the point of silencing the idle run.
  scheduledTasks.push(
  ...
  // Abandoned upload sweep - daily at 3:00 AM, an hour after the run cleanup so
  // the two are not competing for the same disk.
  scheduledTasks.push(
    cron.schedule("0 3 * * *", async () => {
      console.log("[Background Job] Upload sweep cron triggered");
      await processUploadSweep();
    }),
  );

  // Initial MD5 verification 10s after start; unref'd so a pending timer never
  ...
  console.log("  - Abandoned Upload Sweep: Daily at 3:00 AM");
};
...
const stopBackgroundJobs = () => {
  scheduledTasks.forEach((task) => {
    try {
      // destroy(), not stop(): stop() leaves the entry in node-cron's registry
      // to accumulate across repeated initialise/stop cycles.
      ...
      console.error(
        "[Background Jobs] Failed to stop a scheduled task:",
        error,
      );
    }
  });
